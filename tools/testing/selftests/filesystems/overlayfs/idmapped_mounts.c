...
/* A file created through the idmapped mount as ID_NS is stored as ID_HOST. */
TEST_F(idmapped_overlay, create)
{
	...
	/* Caller is uid/gid ID_NS through the mount. */
	fd = openat(ovl, "new", O_CREAT | O_WRONLY | O_EXCL, 0644);
	ASSERT_GE(fd, 0);
	EXPECT_EQ(close(fd), 0);

	ASSERT_EQ(fstatat(ovl, "new", &st, 0), 0);
	EXPECT_EQ(st.st_uid, ID_NS);
	EXPECT_EQ(st.st_gid, ID_NS);

	/* The real upper file is owned by the overlay-final id. */
	snprintf(path, sizeof(path), "%s/u/new", self->base);
